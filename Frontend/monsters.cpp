...
void monstersBegin() {
	...
	earthOne = createMonsterTemplate(EARTH_ONE, earthOneVerts, 20.0f, 4.0f, 2.0f, vec3(EARTH_ONE_SCALE)); // mouse thing
	// earth two
	...
	earthTwo = createMonsterTemplate(EARTH_TWO, earthTwoVerts, 80.0f, 8.0f, 3.0f, vec3(EARTH_TWO_SCALE)); // punchy boi
	getMonsters();
}
...
int createMonsterTemplate(int name, vector<float> vertices,
		float health, float damage, float attackDelayChance, vec3 scale) {
